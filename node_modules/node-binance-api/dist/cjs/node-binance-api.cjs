 ...
 */
    ...
    */
    ...
    futuresKlineConcat(symbol, interval) {
        ...
        if (!this.futuresRealtime[symbol][interval].time)
            return output;
    ...
    futuresKlineHandler(symbol, kline, firstTime = 0) {
        ...
        if (first_updated)
            delete this.futuresTicks[symbol][interval][first_updated];
    ...
    fTickerConvertData(data) {
        const friendlyData = (data) => {
            const { e: eventType, E: eventTime, s: symbol, p: priceChange, P: percentChange, w: averagePrice, c: close, Q: closeQty, o: open, h: high, l: low, v: volume, q: quoteVolume, O: openTime, C: closeTime, F: firstTradeId, L: lastTradeId, n: numTrades } = data;
            return {
                eventType,
                eventTime,
                symbol,
    ...
    fMiniTickerConvertData(data) {
        const friendlyData = (data) => {
            const { e: eventType, E: eventTime, s: symbol, c: close, o: open, h: high, l: low, v: volume, q: quoteVolume } = data;
            return {
                eventType,
                eventTime,
                symbol,
    ...
    fUserDataMarginConvertData(data) {
        const { e: eventType, E: eventTime, cw: crossWalletBalance, // only pushed with crossed position margin call
        // p: positions
         } = data;
        let { positions } = data;
        const positionConverter = position => {
            const { s: symbol, ps: positionSide, pa: positionAmount, mt: marginType, iw: isolatedWallet, // if isolated position
            mp: markPrice, up: unrealizedPnL, mm: maintenanceMargin // maintenance margin required
             } = position;
            return {
                symbol,
                positionSide,
    ...
    fUserDataAccountUpdateConvertData(data) {
        ...
        const updateConverter = updateData => {
            ...
            let { 
            // m: eventReasonType,
            B: balances, P: positions } = updateData;
            const positionConverter = position => {
                const { s: symbol, pa: positionAmount, ep: entryPrice, cr: accumulatedRealized, // (Pre-fee) Accumulated Realized
                up: unrealizedPnL, mt: marginType, iw: isolatedWallet, // if isolated position
                ps: positionSide } = position;
                return {
                    symbol,
                    positionAmount,
                    entryPrice,
    ...
    fUserDataOrderUpdateConvertData(data) {
        const { e: eventType, E: eventTime, T: transaction, // transaction time
         } = data;
        let { o: order } = data;
        const orderConverter = (order) => {
            const { s: symbol, c: clientOrderId, 
            ...
            wt: stopPriceWorkingType, ot: originalOrderType, ps: positionSide, cp: closeAll, // if close-all, pushed with conditional order
    ...
    fMarkPriceConvertData(data) {
        const friendlyData = (data) => {
            const { e: eventType, E: eventTime, s: symbol, p: markPrice, i: indexPrice, r: fundingRate, T: fundingTime } = data;
            return {
                eventType,
                eventTime,
                symbol,
    ...
    fAggTradeConvertData(data) {
        const friendlyData = (data) => {
            const { e: eventType, E: eventTime, s: symbol, a: aggTradeId, p: price, q: amount, f: firstTradeId, l: lastTradeId, T: timestamp, m: maker } = data;
            return {
                eventType,
                eventTime,
                symbol,
    ...
    deliveryKlineConcat(symbol, interval) {
        ...
        if (typeof this.deliveryRealtime[symbol][interval].time === 'undefined')
            return output;
    ...
    deliveryKlineHandler(symbol, kline, firstTime = 0) {
        ...
        if (first_updated)
            delete this.deliveryTicks[symbol][interval][first_updated];
    ...
    dMarkPriceConvertData(data) {
        const friendlyData = (data) => {
            const { e: eventType, E: eventTime, s: symbol, p: markPrice, r: fundingRate, T: fundingTime } = data;
            return {
                eventType,
                eventTime,
                symbol,
   ...
   */
    dUserDataOrderUpdateConvertData(data) {
        ...
        const orderConverter = (order) => {
            ...
            // "adl_autoclose": ADL auto close order
            S: side, o: orderType, f: timeInForce, q: originalQuantity, p: originalPrice, ap: averagePrice, sp: stopPrice, // please ignore with TRAILING_STOP_MARKET order,
            x: executionType, X: orderStatus, i: orderId, l: orderLastFilledQuantity, z: orderFilledAccumulatedQuantity, L: lastFilledPrice, ma: marginAsset, N: commissionAsset, // will not push if no commission
            n: commission, // will not push if no commission
            T: orderTradeTime, t: tradeId, rp: realizedProfit, b: bidsNotional, a: askNotional, m: isMakerSide, // is this trade maker side
            R: isReduceOnly, // is this reduce only
            wt: stopPriceWorkingType, ot: originalOrderType, ps: positionSide, cp: closeAll, // if close-all, pushed with conditional order
            AP: activationPrice, // only pushed with TRAILING_STOP_MARKET order
            cr: callbackRate, // only pushed with TRAILING_STOP_MARKET order
   ...
   */
    ...
    prevDayConvertData(data) {
        const convertData = (data) => {
            const { e: eventType, E: eventTime, s: symbol, p: priceChange, P: percentChange, w: averagePrice, x: prevClose, c: close, Q: closeQty, b: bestBid, B: bestBidQty, a: bestAsk, A: bestAskQty, o: open, h: high, l: low, v: volume, q: quoteVolume, O: openTime, C: closeTime, F: firstTradeId, L: lastTradeId, n: numTrades } = data;
            return {
                eventType,
                eventTime,
                symbol,
    ...
    klineHandler(symbol, kline, firstTime = 0) {
        ...
        if (!isFinal) {
            if (typeof this.ohlcLatest[symbol][interval].time !== 'undefined') {
                if (this.ohlcLatest[symbol][interval].time > time)
                    return;
            }
        ...
        if (first_updated)
            delete this.ohlc[symbol][interval][first_updated];
        this.ohlc[symbol][interval][time] = { open: open, high: high, low: low, close: close, volume: volume };
    }
    /**
    ...
    * @return {promise or undefined} - omitting the callback returns a promise
    */
    async candlesticks(symbol, interval = '5m', params = {}) {
        if (!params.limit)
            params.limit = 500;
    ...
    * @return {promise or undefined} - omitting the callback returns a promise
    */
    async candles(symbol, interval = '5m', params = {}) {
    ...
    */
    async futuresCandles(symbol, interval = "30m", params = {}) {
        params.symbol = symbol;
     ...
     * @see https://developers.binance.com/docs/derivatives/usds-margined-futures/market-data/rest-api/Kline-Candlestick-Data
    */
    async futuresCandlesticks(symbol, interval = "30m", params = {}) {
 ...
 */
    ...
    futuresChart(symbols, interval, callback, limit = 500) {
        const reconnect = () => {
            if (this.Options.reconnect)
                this.futuresChart(symbols, interval, callback, limit);
        };
        const futuresChartInit = (symbol) => {
            if (typeof this.futuresMeta[symbol] === 'undefined')
                this.futuresMeta[symbol] = {};
            if (typeof this.futuresMeta[symbol][interval] === 'undefined')
                this.futuresMeta[symbol][interval] = {};
            if (typeof this.futuresTicks[symbol] === 'undefined')
                this.futuresTicks[symbol] = {};
            if (typeof this.futuresTicks[symbol][interval] === 'undefined')
                this.futuresTicks[symbol][interval] = {};
            if (typeof this.futuresRealtime[symbol] === 'undefined')
                this.futuresRealtime[symbol] = {};
            if (typeof this.futuresRealtime[symbol][interval] === 'undefined')
                this.futuresRealtime[symbol][interval] = {};
        ...
        const handleFuturesKlineStream = kline => {
            ...
            if (!this.futuresMeta[symbol][interval].timestamp) {
                ...
                if (callback)
                    callback(symbol, interval, this.futuresKlineConcat(symbol, interval));
            }
        };
        const getFuturesKlineSnapshot = async (symbol, limit = 500) => {
            ...
            if (typeof this.futuresKlineQueue[symbol][interval] !== 'undefined') {
                for (const kline of this.futuresKlineQueue[symbol][interval])
                    this.futuresKlineHandler(symbol, kline, this.futuresMeta[symbol][interval].timestamp);
     ...
     * @return {string} the websocket endpoint
     */
    futuresCandlesticksStream(symbols, interval, callback) {
        const reconnect = () => {
            if (this.Options.reconnect)
                this.futuresCandlesticksStream(symbols, interval, callback);
        };
        let subscription;
        if (Array.isArray(symbols)) {
            if (!this.isArrayUnique(symbols))
    ...
    deliveryChart(symbols, interval, callback, limit = 500) {
        const reconnect = () => {
            if (this.Options.reconnect)
                this.deliveryChart(symbols, interval, callback, limit);
        };
        const deliveryChartInit = symbol => {
            if (typeof this.deliveryMeta[symbol] === 'undefined')
                this.deliveryMeta[symbol] = {};
            if (typeof this.deliveryMeta[symbol][interval] === 'undefined')
                this.deliveryMeta[symbol][interval] = {};
            if (typeof this.deliveryTicks[symbol] === 'undefined')
                this.deliveryTicks[symbol] = {};
            if (typeof this.deliveryTicks[symbol][interval] === 'undefined')
                this.deliveryTicks[symbol][interval] = {};
            if (typeof this.deliveryRealtime[symbol] === 'undefined')
                this.deliveryRealtime[symbol] = {};
            if (typeof this.deliveryRealtime[symbol][interval] === 'undefined')
                this.deliveryRealtime[symbol][interval] = {};
            ...
            if (typeof this.deliveryKlineQueue[symbol][interval] === 'undefined')
                this.deliveryKlineQueue[symbol][interval] = [];
        ...
        const handleDeliveryKlineStream = kline => {
            ...
            if (!this.deliveryMeta[symbol][interval].timestamp) {
                ...
                if (callback)
                    callback(symbol, interval, this.deliveryKlineConcat(symbol, interval));
            }
        };
        const getDeliveryKlineSnapshot = async (symbol, limit = 500) => {
            ...
            if (typeof this.deliveryKlineQueue[symbol][interval] !== 'undefined') {
                for (const kline of this.deliveryKlineQueue[symbol][interval])
                    this.deliveryKlineHandler(symbol, kline, this.deliveryMeta[symbol][interval].timestamp);
    ...
    deliveryCandlesticks(symbols, interval, callback) {
        const reconnect = () => {
            if (this.Options.reconnect)
                this.deliveryCandlesticks(symbols, interval, callback);
        };
        let subscription;
        if (Array.isArray(symbols)) {
            if (!this.isArrayUnique(symbols))
    ...
    userFutureData(all_updates_callback, margin_call_callback, account_update_callback, order_update_callback, subscribed_callback, account_config_update_callback) {
        ...
        this.apiRequest(url + 'v1/listenKey', {}, 'POST').then((response) => {
   ...
   */
    ...
    depthCacheStream(symbols, callback, limit = 500) {
        ...
        const getSymbolDepthSnapshot = async (symbol) => {
    ...
    */
    ...
    chart(symbols, interval, callback, limit = 500) {
        ...
        const handleKlineStreamData = kline => {
            ...
            if (!this.info[symbol][interval].timestamp) {
                ...
                if (callback)
                    callback(symbol, interval, this.klineConcat(symbol, interval));
            }
        };
        const getSymbolKlineSnapshot = async (symbol, limit = 500) => {
            ...
            if (typeof this.klineQueue[symbol][interval] !== 'undefined') {
                for (const kline of this.klineQueue[symbol][interval])
                    this.klineHandler(symbol, kline, this.info[symbol][interval].timestamp);
                delete this.klineQueue[symbol][interval];
            }
            if (callback)
                callback(symbol, interval, this.klineConcat(symbol, interval));
    ...
    candlesticksStream(symbols, interval, callback) {
        const reconnect = () => {
            if (this.Options.reconnect)
                this.candlesticksStream(symbols, interval, callback);
        };
        /* If an array of symbols are sent we use a combined stream connection rather.
         This is transparent to the developer, and results in a single socket connection.
         This essentially eliminates "unexpected response" errors when subscribing to a lot of data. */
